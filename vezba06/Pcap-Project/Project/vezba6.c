...
int main()
{
    ...
    while((result = pcap_next_ex(device_handle, &packet_header, &packet_data)) >= 0){
        ...
        // Retrieve the position of the data
        app_data = (unsigned char*)uh + sizeof(udp_header);
        data_len = ntohs(uh->datagram_length) - sizeof(udp_header);
        print_application_data(app_data, ntohs(uh->datagram_length));
